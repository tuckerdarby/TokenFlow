...
def sa_forward(self, n_conds=1, n_unconds=1):

    # self.scale = 1  #  0.18215  # 1  # TODO
    # to_out is good
    to_out = self.to_out
    ...
    def forward2(x, context=None, value=None, mask=None):
        ...
        batch_size, sequence_length, dim = x.shape
        h = self.heads
        n_frames = batch_size // (1 + n_conds + n_unconds)
        is_cross = encoder_hidden_states is not None
        encoder_hidden_states = encoder_hidden_states if is_cross else x
        ...
        if self.injection_schedule is not None and (self.t in self.injection_schedule or self.t >= 999):
            for idx in range(n_conds + n_unconds):
        ...
        for idx in range(1 + n_conds + n_unconds):
